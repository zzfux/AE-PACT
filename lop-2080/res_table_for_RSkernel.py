...
def add_features(K, H, C, S, Stride, Tk, Th, Tw, Bk, Bh, Bw, CMult, alt, machine_info):
    ...
    sharedMemoryIn = (Bh * Th + R - 1) * (Bw * Tw + S - 1)
    ...
    if alt == 1:
        return [K, H, C, S, Stride, Tk, Th, Tw, Bk, Bh, Bw, CMult]
   
    elif alt == 2:
        return [K, H, C, S, Stride, Tk, Th, Tw, Bk, Bh, Bw, CMult, Ops, G2S_trans, S2R_trans, S2G_trans, R2S_trans, G2R_kernel, recp_occupancy, correct_wave, conc_tb,warpsPerBlock, num_TB, totalShared, totalRegistersUsedBlock]
...
def expand_csv(file, alt, stride, if_train, if_norm, pass_norm_factor, machine_info):
    ...
    if alt == 1:
        new_header = "K, H, C, S, Stride, Tk, Th, Tw, Bk, Bh, Bw, CMult, Time"
    elif alt == 2:  # problem sizes, stride, tile sizes, OPs, GM-transactions, SM-transactions,
        new_header = "K, H, C, S, Stride, Tk, Th, Tw, Bk, Bh, Bw, CMult, Ops, G2S_trans, S2R_trans, S2G_trans, R2S_trans, G2R_kernel, recp_occupancy, correct_wave, conc_tb,warpsPerBlock, num_TB, totalShared, totalRegistersUsedBlock, Time"
...
def RS3_buildML(dirpath, machine_info,  topN, step, initN, train_set_file, indepent_test_set_file, split_ratio, list_random_state, algo_list, algo_name, template_name):
    ...
    for alt in alt_list:
        ...
        for al in algo_list:
            ...
            for seed in list_random_state:
                ...
                # model_name = algo_name[i]+"_"+str(seed)+"_"+str(alt)+".pkl"
                # model = copy.deepcopy(regressor)
                # if algo_name[i] in model_dict:
                #     model_dict[algo_name[i]].append( (model_name, model))
                # else:
                #     model_dict[algo_name[i]] = []
                #     model_dict[algo_name[i]].append( (model_name, model))
...
def RS3(dirpath, machine_info,  topN, step, initN, all_model_dict, algo_list, algo_name):
    print("working path ", dirpath)
    stride = 1
